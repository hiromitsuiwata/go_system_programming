...
func main() {
	file, err := os.Open("Lenna_new.png")
	if err != nil {
		panic(err)
	}
	defer file.Close()

	newFile, err := os.Create("Lenna_new2.png")
	if err != nil {
		panic(err)
	}
	defer newFile.Close()
	...
	// io.Copy(newFile, textChunk("Hello, worldaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"))
